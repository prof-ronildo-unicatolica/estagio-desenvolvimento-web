...
* **Tamanho das Equipes:** 2 a 4 alunos. Este tamanho é ideal para evitar o "efeito carona" (freeloading), permitir a divisão clara de papéis (ex: Frontend, Backend, Banco de Dados) e facilitar o trabalho em pares (*Pair Programming*).
* **Posse e Controle dos Repositórios (GitHub):** Os repositórios oficias de cada projeto (ex: `turma_a_equipe_a`) serão criados e mantidos sob administração direta do professor (preferencialmente através de uma *GitHub Organization* dedicada à disciplina).
